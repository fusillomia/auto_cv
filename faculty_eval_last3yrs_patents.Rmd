...
```{r patents}
patents <- all_works[types %in% c("patent", "license"),]
from_lens <- !map_lgl(patents$`external-ids.external-id`, is_empty)
patents$pat[from_lens] <- map_chr(
  patents$`external-ids.external-id`[from_lens],
  ~ .x[.x$`external-id-type` == "pat", "external-id-value"]
)
manual <- str_split(
  patents$`journal-title.value`[!from_lens & patents$type == "patent"],
  "\\. "
)
patents$pat[!from_lens & patents$type == "patent"] <- map(manual, 1) %>%
  map_chr(~ gsub("PAT: ", "", .x))
patents$pat[is.na(patents$pat)] <- " "
patents$office <- patents$`journal-title.value`
patents$office[!from_lens & patents$type == "patent"] <- map_chr(manual, 2)
patents$office <- case_when(
      !is.na(patents$office) ~ patents$office,
      grepl("^EP ", patents$pat) ~ "European Patent Office",
      grepl("^US ", patents$pat) ~ "United States Patent and Trademark Office"
    )
patents %>%
    detailed_entries(
      with = office,
      ...
      what = paste(type, title.title.value, sep = ": ")
    )
```
